        ...
        <div class="container px-5 py-16 mx-auto">
            ...
            @foreach ($clientes as $cliente)
                <div class="text-center">
                    ...
                    <p class="text-base leading-relaxed xl:w-2/4 lg:w-3/4 mx-auto text-gray-500s dark:text-white">
                        {{ $cliente->nro_rif }}
                    </p>
            ...
            <div class="flex flex-wrap -m-4 mt-4">
                <div class="p-4 lg:w-1/3">
                    <div class="h-full bg-gray-100 bg-opacity-75 px-8 pt-16  rounded-lg overflow-hidden text-center relative">
                        {{-- DB gestionweb esq. public tbl dst_detailsfactura --}}
                        @include('gestionweb.detailsFactura')
                    </div>
                </div>
                <div class="p-4 lg:w-1/3">
                    <div class="h-full bg-gray-100 bg-opacity-75 px-8 pt-16  rounded-lg overflow-hidden text-center relative">
                        {{-- DB gestionweb esq. public tbl dst_detallesolicitud --}}
                        @include('gestionweb.detalleSolicitud')
                    </div>
                </div>
                <div class="p-4 lg:w-1/3">
                    <div class="h-full bg-gray-100 bg-opacity-75 px-8 pt-16  rounded-lg overflow-hidden text-center relative">
                        {{-- DB gestionweb esq. public tbl dst_asignaciones --}}
                        @include('gestionweb.asignacion')
                    </div>
                </div>
                @if (!empty($tickets))
                    <div class="p-4 lg:w-1/3">
                        <div class="h-full bg-gray-100 bg-opacity-75 px-8 pt-16  rounded-lg overflow-hidden text-center relative">
                            {{-- DB mckoi esq. reg044 tbl ticket --}}
                            @include('mckoi.reversoDosTicket')
                        </div>
                    </div>
                @endif
                <div class="p-4 lg:w-1/3">
